...
public class Pagina implements IPagina{
    ...
    public void getHead() throws SQLException {
        ...
        if(rs.next()) {
          ...
          sql = "SELECT b.NombreFicheroCss as css, b.Path as ruta  FROM `pagina_css` a, fichero_css b"
           + " WHERE a.`IdFicheroCss` = b.IdFicheroCss and `IdPagina` =" +   this.idPagina;
    ...
    public void  getJs() throws SQLException {
        ...
        String sql = "SELECT a.`NombreFicheroJs` as js,  a.`Path` as ruta, b.`OrdenJs`  FROM `fichero_js` a, pagina_js b"
                + " WHERE a.`IdFicheroJs` = b.IdFicheroJs AND b.IdPagina =" +   this.idPagina + " order by 3";

        rs = conexion.executeQuery(sql);
        while(rs.next()){
            this.body = this.body + "<script src='"+rs.getString("ruta")+"/"+rs.getString("js")+".js' charset=\"UTF-8\"></script>";
